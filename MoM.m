...
%              [obj.G1, obj.G2] = DistanceSetup(obj, ant);
        ...
        function [obj, ant, area] = mombasis(obj, ant, area, alpha, k, w, thetaI, phi, phiS, mu) 
            ...
            Z2 = ant.T1(:)*ant.T2(:).'.*...
                (sin(ant.gammaTest(1:end-1))*sin(ant.gammaTest(2:end).').*obj.G2(:,:) ...%1
                +cos(ant.gammaTest(1:end-1))*cos(ant.gammaTest(2:end).').*obj.G1(:,:)) ...
                -1./k.^2.*ant.T1D(:)*ant.T2D(:).'.*obj.G1(:,:);
            Z3 = ant.T2(:)*ant.T2(:).'.*...
                (sin(ant.gammaTest(2:end))*sin(ant.gammaTest(2:end).').*obj.G2(:,:) ...%1
                +cos(ant.gammaTest(2:end))*cos(ant.gammaTest(2:end).').*obj.G1(:,:)) ...
                -1./k.^2.*ant.T2D(:)*ant.T2D(:).'.*obj.G1(:,:);
            Z4 = ant.T2(:)*ant.T1(:).'.*...
                (sin(ant.gammaTest(2:end))*sin(ant.gammaTest(1:end-1).').*obj.G2(:,:) ...%1
...
%                 -1./k.^2.*ant.T2D(:)*ant.T1D(:).'.*obj.G1(:,:);   
            ...
            b1=-ant.E0(2:end-1).*1i./(w.*mu).*pi.*1i.^(alpha).*...
            ...
            b2=-ant.E0(2:end-1).*1i./(w.*mu).*pi.*1i.^(alpha).*...
            ...
            ant.xtTheta = [ant.xtTheta];
            ant.btTheta = [ant.btTheta];
