...
xNulAlphaThe=xtthe.*ftn;%.*tHat;
xNulAlphaPhi=xtphi.*fpn;%.*tHat;  
...
xstep = 100;
zstep = 100;
x = linspace(-length*2, length*2, xstep);
z = linspace(-length, length, zstep);
Ethe = x.*z';
Ethe(:,:) = 0;
Ephi = Ethe;
rz = (coord(:,1)-z);
...
for alpha=1:2
    ...
    fpn = (T1+T2).*exp(1i.*alpha.*0);%Phi, alpha, n. Expansions function
    phi=0;
    Jthe = xNulAlphaThe+2*sum(xtthe.*ftn.*cos(alpha.*phi)+xphithe.*ftn.*sin(alpha.*phi))+Jthe;
    Jphi = xNulAlphaPhi+2*sum(xtphi.*fpn.*sin(alpha.*phi)+xphiphi.*fpn.*cos(alpha.*phi))+Jphi;
    phiS = 0;
    ...
    for i=1:N    
        ...
        + B*(xtthe(i)*btthe(i)+xphithe(i)*bphithe(i))*cos(alpha*phiS)+Ethethe;
