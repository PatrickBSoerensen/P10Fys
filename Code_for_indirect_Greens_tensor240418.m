% Code for calculating the indirect Greens tensor   
    ...
    rpf=@(krho) (rp1L2f(krho)+rpL2L3f(krho).*exp(2i*kzL2f(krho)*layer_thickness))./(1+rp1L2f(krho).*rpL2L3f(krho).*exp(2i*kzL2f(krho)*layer_thickness));
    rsf=@(krho) (rs1L2f(krho)+rsL2L3f(krho).*exp(2i*kzL2f(krho)*layer_thickness))./(1+rs1L2f(krho).*rsL2L3f(krho).*exp(2i*kzL2f(krho)*layer_thickness));
