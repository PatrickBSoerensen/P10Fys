...
%                                 *sum(sum(repmat(zMP,[Quad,1]).* zNP_/4-1/k^2,2) .* (1i*k+I2(y))/Quad)...
%                                 + Z(PO(i), PI(j));
%                             
%                                 Z(PO(i), PI(j)) = ...
%                                 (BasisLA(PO(i),2)*BasisLA(PI(j),2))/(4*pi)...
%                                 *sum(sum(zMP_.* repmat(zNP,[Quad,1])/4-1/k^2,2) .*(1i*k+I2(y))/Quad)...
%                                 + Z(PO(i), PI(j));
%                             else
                                Z(PO(i), PI(j)) = ...
                                (BasisLA(PO(i),2)*BasisLA(PI(j),2))/(4*pi)...
                                *sum(sum(repmat(zMP,[Quad,1]).* zNP_/4-1/k^2,2) .* gPPo/Quad)...
                                + Z(PO(i), PI(j));
                            
                                Z(PO(i), PI(j)) = ...
                                (BasisLA(PO(i),2)*BasisLA(PI(j),2))/(4*pi)...
                                *sum(sum(zMP_.* repmat(zNP,[Quad,1])/4-1/k^2,2) .* gPPi/Quad)...
                                + Z(PO(i), PI(j));
%                             end
                        end
                        for j=1:length(MI)
...
%                                 *sum(sum(repmat(zMP,[Quad,1]).* zNM_ /4+1/k^2,2) .* (1i*k+I2(y))/Quad)...
%                                 + Z(PO(i), MI(j));
%                                 
%                                 Z(PO(i), MI(j)) = ...
%                                 (BasisLA(PO(i),2)*BasisLA(MI(j),2))/(4*pi)...
%                                 *sum(sum(zMP_.* repmat(zNM,[Quad,1])/4+1/k^2,2) .* (1i*k+I2(y))/Quad)...
%                                 + Z(PO(i), MI(j));
%                             else
                                Z(PO(i), MI(j)) = ...
                                (BasisLA(PO(i),2)*BasisLA(MI(j),2))/(4*pi)...
                                *sum(sum(repmat(zMP,[Quad,1]).* zNM_ /4+1/k^2,2) .* gPMo/Quad)...
                                + Z(PO(i), MI(j));
                                
                                Z(PO(i), MI(j)) = ...
                                (BasisLA(PO(i),2)*BasisLA(MI(j),2))/(4*pi)...
                                *sum(sum(zMP_.* repmat(zNM,[Quad,1])/4+1/k^2,2) .* gMPi/Quad)...
                                + Z(PO(i), MI(j));
%                                 end  
                        end
...
%                                 *sum(sum(repmat(zMM,[Quad,1]).* zNP_/4+1/k^2,2) .* (1i*k+I2(y))/Quad)...
%                                 + Z(MO(i), PI(j));
%                             
%                                 Z(MO(i), PI(j)) = ...
%                                 (BasisLA(MO(i),2)*BasisLA(PI(j),2))/(4*pi)...
%                                 *sum(sum(zMM_.* repmat(zNP,[Quad,1])/4+1/k^2,2) .*(1i*k+I2(y))/Quad)...
%                                 + Z(MO(i), PI(j));  
%                             else
                                Z(MO(i), PI(j)) = ...
                                (BasisLA(MO(i),2)*BasisLA(PI(j),2))/(4*pi)...
                                *sum(sum(repmat(zMM,[Quad,1]).* zNP_/4+1/k^2,2) .* gMPo/Quad)...
                                + Z(MO(i), PI(j));
                            
                                Z(MO(i), PI(j)) = ...
                                (BasisLA(MO(i),2)*BasisLA(PI(j),2))/(4*pi)...
                                *sum(sum(zMM_.* repmat(zNP,[Quad,1])/4+1/k^2,2) .* gPMi/Quad)...
                                + Z(MO(i), PI(j));  
%                                    end
                        end
                        for j=1:length(MI)
...
%                                 *sum(sum(repmat(zMM,[Quad,1]).* zNM_/4-1/k^2,2) .* (1i*k+I2(y))/Quad)...
%                                 + Z(MO(i), MI(j));
%                             
%                                 Z(MO(i), MI(j)) = ...
%                                 (BasisLA(MO(i),2)*BasisLA(MI(j),2))/(4*pi)...
%                                 *sum(sum(zMM_.* repmat(zNM,[Quad,1])/4-1/k^2,2) .*(1i*k+I2(y))/Quad)...
%                                 + Z(MO(i), MI(j));
%                             else
                                Z(MO(i), MI(j)) = ...
                                (BasisLA(MO(i),2)*BasisLA(MI(j),2))/(4*pi)...
                                *sum(sum(repmat(zMM,[Quad,1]).* zNM_/4-1/k^2,2) .* gMMo/Quad)...
                                + Z(MO(i), MI(j));
                            
                                Z(MO(i), MI(j)) = ...
                                (BasisLA(MO(i),2)*BasisLA(MI(j),2))/(4*pi)...
                                *sum(sum(zMM_.* repmat(zNM,[Quad,1])/4-1/k^2,2) .* gMMi/Quad)...
                                + Z(MO(i), MI(j));
%                                     end
                        end
...
%             rhov1 = [rhomin rhomin+dx rhomin+dx*sqrt(2) rhomin+dx*2 rhomin+dx*sqrt(5) rhomin+dx*sqrt(8)];
        ...
        function [SelfP, SelfM] = NearTriangleZ(p, t, EdgeList)
            ...
            for i=1:length(EdgeList)
                ...
                I11 = log((b+sqrt(a).*sqrt(c))./(b-c-sqrt(c).*sqrt(a-2*b+c)))./(40*sqrt(c))...
                ...
                I12 = log((b+sqrt(a).*sqrt(c))./(b-c+sqrt(c).*sqrt(a-2*b+c)))./(120*sqrt(c))...
                ...
                I  = (-log((-b+sqrt(a).*sqrt(c))./(a-b+sqrt(a).*sqrt(a-2*b+c))))./(24*sqrt(a))...
            ...
            if j==1
            SelfP(i) = -(I11*T1+I11*T3+(I12*T2+I12*T4)+(I*T5+I*T6+I*T7+I*T8+T9));
            elseif j==2
            SelfM(i) = I11*T1+I11*T3+(I12*T2+I12*T4)+(I*T5+I*T6+I*T7+I*T8+T9);
            end
            end
            end
        end
