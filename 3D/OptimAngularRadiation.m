...
[pclean, tclean] = ArbitraryAntenna.RemoveDuplicatePoints(stl);
%%
...
pclean(:,3) = pclean(:,3)+AntFromReflector;
...
p2 = pclean;
% p3 = p;
p1(:,1) = p1(:,1)-dist(DistIndex);
p2(:,1) = p2(:,1)+dist(DistIndex);
p = [p1; p2];%; p3; p4];
t = [tclean; tclean+length(p1)];% t+length(p1)+length(p2); t+length(p1)+length(p2)+length(p3)];
% p(:,1) = p(:,1)+0.03;
...
save(FileName, 'UseDipole', 'SubSubTri', 'vectorized', 'ESCAng', 'EscRef', 'EzyFull', 'ExzFull');
